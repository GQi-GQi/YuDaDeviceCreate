...
namespace YuDa_DeviceCreate
{
    ...
    public class DetectCom
    {
        ...
        public DetectCom(string[] bakSerialPortList)
        {
            DetectComMode = DetectComModeEnum.None;
            DetectComInterval = DETECT_COM_INTERVAL;

            if (bakSerialPortList != null)
                this.bakSerialPortList = bakSerialPortList.ToList();
        }
        #endregion
        ...
        {
            //判断事件是否为空
            if (this.AddSerialPortCallback != null || this.DelSerialPortCallback != null)
            {
                bool ret = false;
                //根据检测模式开启检测
        ...
        /// 串口新增回调
        /// </summary>
        public Action<List<string>> AddSerialPortCallback;

        /// <summary>
        /// 串口移除回调
        /// </summary>
        public Action<List<string>> DelSerialPortCallback;
        ...
        private void DetectSerialPortListProcess()
        {
            try
            {
                ...
                nowSerialPortList.AddRange(SerialPort.GetPortNames());
                ...
                if (CompareArray(nowSerialPortList.ToArray(), bakSerialPortList.ToArray()) == false)
                {
                    ...
                    List<string> addSerialPort = nowSerialPortList.Except(bakSerialPortList.ToArray()).ToList();
                    List<string> delSerialPort = bakSerialPortList.Except(nowSerialPortList.ToArray()).ToList();

                    //触发事件
                    if (addSerialPort.Count > 0 && AddSerialPortCallback != null)
                        AddSerialPortCallback(addSerialPort);
                    if (delSerialPort.Count > 0 && DelSerialPortCallback != null)
                        DelSerialPortCallback(delSerialPort);

                    bakSerialPortList.AddRange(addSerialPort);
                    foreach (string serialPort in delSerialPort)
                        bakSerialPortList.Remove(serialPort);
                }
            }
            catch
            {
